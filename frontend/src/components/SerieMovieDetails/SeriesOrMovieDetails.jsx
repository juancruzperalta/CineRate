    ...
    <> 
      ...
      <div className='max-h-full w-full h-full z-10 items-start justify-center flex flex-col overflow-hidden lg:px-0'>
        <div className="pt-1 flex flex-col text-[0.9rem] text-gray-300 max-w-full">
          <h2 className='items-start text-start flex text-3xl font-semibold text-gray-100 border-b-2 border-gray-600'>{serie ? currentDetails?.name : currentDetails?.title}</h2>
            <span className='items-start flex text-[var(--colorAccent)] text-[1rem]  font-semibold'>{serie ? currentDetails?.first_air_date : currentDetails?.release_date}</span>
        </div>
        <div className="flex flex-wrap items-center text-[1rem]  gap-2 text-gray-300/70">
