...
// Lo que hacemos acá es que : los detalles de la serie los pide según verdadero o true (si es serie o pelicula), entonces pide, hay que hacer el llamado a movies. (En backend)
  return (
    <> 
      <div className='lg:grid lg:grid-cols-2 ml-1 flex flex-col justify-center h-full items-center overflow-hidden lg:text-[0.9rem] text-[0.8rem] 2xl:text-[1rem]'>
      <div className='max-h-full h-full z-10 items-start justify-center flex flex-col overflow-hidden w-full px-4 lg:px-0'>
        <div className="pt-1 flex flex-col w-full text-[0.9rem] text-gray-300 max-w-full">
          <h2 className='items-start text-start flex text-2xl font-semibold text-gray-100'>{serie ? currentDetails?.name : currentDetails?.title}</h2>
            <span className='items-start flex '>{serie ? currentDetails?.first_air_date : currentDetails?.release_date}</span>
        </div>
        <p className='w-full lg:w-[300px] text-[0.9rem] line-clamp-4 items-start text-start'>{currentDetails?.overview}</p>
       <div className="flex flex-wrap items-center  gap-2 text-gray-300">
        ...
        <div className='mt-2 lg:mt-0 max-h-full h-full z-10 relative items-center justify-center flex flex-col '>
