    ...
    <>
      ...
      {premiereMovies?.results.slice(0,cant).map(movie => (
        ...
        <div key={movie?.id} className='flex h-full flex-col w-full items-center justify-center'>
          ...
          : (  <div key={movie?.id} className='flex h-full flex-col w-full items-center justify-center'>
         <div className='max-w-full min-w-full object-cover rounded-lg shadow-md cursor-pointer bg-gray-700 hover:opacity-80' onClick={() => navigate(`/movies/details/${movie?.id}`)}></div>
          <span className='max-w-full min-w-full whitespace-nowrap text-ellipsis line-clamp-1  text-gray-200 font-bold'>{movie?.title}</span>
        </div>)
      ))}
      </div>
        <div className='h-20'>
