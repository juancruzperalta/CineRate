...
export const MoviesPopulars = () => {
  ...
  return (
    ...
    <div className='flex flex-row relative '>
      ...
      {movies.map((movie) => (
        <div key={movie.id} className={`relative min-w-[200px] transition-all duration-500 overflow-hidden  ${
    DetailsID === null
      ? 'opacity-100'
      : movie.id === DetailsID
        ? 'opacity-100'
        : 'opacity-35'
          }`} onMouseEnter={() => { clearTimeout(leaveTimer), viewDetails(movie.id) }}
      onMouseLeave={() => {
  leaveTimer = setTimeout(() => viewDetails(null), 200);
}}>
          <img key={movie.id} src={`https://image.tmdb.org/t/p/w500${movie.backdrop_path}`} alt={movie.name} className='h-[300px] object-cover rounded-lg shadow-md cursor-pointer ' onClick={() => navigate(`/series/details/${movie.id}`)} />
        <div
          className="absolute bottom-0 w-full bg-[var(--bgSecondary)]/90 flex flex-col items-center transition-all duration-300  rounded-b-lg py-2 px-2 border-t-1 border-gray-300/45 shadow-md max-w-full overflow-hidden whitespace-nowrap text-ellipsis"
            style={{
              display: '-webkit-box',
    WebkitLineClamp: movie.id === DetailsID ? 6 : 1,
    WebkitBoxOrient: 'vertical',
          maxHeight: movie.id === DetailsID ? '300px' : '50px',
          opacity: movie.id === DetailsID ? 1 : 0.9,
          }}
        >
         ...
         <div
          className={`transition-all duration-500 ${
            ...
            >
{movie.id === DetailsID && (
  <MovieCardDetails movieId={movie.id} />
)}
          </div>
