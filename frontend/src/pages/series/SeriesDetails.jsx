...
export const SeriesDetails = () => {
  ...
  useEffect(() => {
    ...
    if (isLoading) {
      return (
      <>
  <div className="flex flex-col mb-4 h-full w-full px-10 min-h-100vh w-full overflow-hidden ">
            <div className='mt-24 w-full h-[40px] justify-around items-center bg-gray-700/50 animate-pulse  p-1'>
          
              </div>
            <div className='w-full h-[400px] min-h-[400px] max-h-full flex flex-col md:grid md:grid-cols-2 overflow-hidden mt-4 items-center justify-center gap-6 relative'>
        ...
        <div className="h-full w-full bg-gray-700/50 rounded animate-pulse mb-2"></div>
            ...
            <div className="mt-6 h-4 p-1 md:w-240 bg-gray-700/50 rounded animate-pulse"></div>
  ...
  return (
    <div className='flex flex-col  mb-4 h-full min-h-100vh  w-full px-10  overflow-hidden'>
          <div className='mt-24 w-full flex flex-col md:grid md:grid-cols-3 justify-around items-center border-1 border-gray-200/40 bg-gray-600/20 p-1'>
            <span className='text-start md:text-3xl text-xl text-gray-200 font-bold uppercase'>{currentSerieDetails?.name}</span>
            ...
            <div className='max-w-full w-full h-full min-h-[400px] max-h-full flex flex-col md:grid md:grid-cols-2 overflow-hidden mt-2 items-center justify-center gap-6 relative'>
              ...
              <span className={`hidden md:flex absolute text-sm bottom-1 p-[0.23rem] left-1 ${currentSerieDetails?.in_production ? 'bg-green-300' : 'hidden'} text-black font-semibold`}>{currentSerieDetails?.in_production ? "In Production" : ""}</span>
            </div>
            ...
            <div className='flex flex-col items-start justify-start gap-1 mt-4'>
              ...
              <div className=' flex flex-col w-full  items-center justify-center gap-1  mt-4'>
                ...
                <div className={`grid ${isVisible ? `grid-rows-${similarSerie.length / 8}` : 'grid-rows-1'} gap-4 items-center justify-center w-full mt-2`} style={{gridTemplateColumns: `repeat(${getCant()}, minmax(0,1fr)`}}>
                  {visible?.length > 0 ? visible?.map(serie => (
                    <img key={serie.id} src={`https://image.tmdb.org/t/p/original${serie?.poster_path}`} alt={serie?.name} onClick={() => navigate(`/series/details/${serie.id}`)} className='w-full h-[200px] rounded-md object-center cursor-pointer' />
                  )
                )
                    :
                    <>
