...
<div className="relative flex items-center justify-center w-full h-[100vh] overflow-hidden p-0 m-0   mb-6 bg-[#090c0f] overflow-y-hidden">
      ...
      <div className="h-4 w-32 bg-gray-700 rounded animate-pulse"><span className='opacity-30'>Loading...</span></div>
      <div className="h-8 w-60 bg-gray-700 rounded animate-pulse"><span className='opacity-30'>Loading...</span></div>
      <div className="flex gap-2">
      <div className="h-6 w-10 bg-gray-700 rounded animate-pulse"><span className='opacity-30'>Loading...</span></div>
      <div className="h-6 w-10 bg-gray-700 rounded animate-pulse"><span className='opacity-30'>Loading...</span></div>
      <div className="h-6 w-10 bg-gray-700 rounded animate-pulse"><span className='opacity-30'>Loading...</span></div>
      <div className="h-6 w-10 bg-gray-700 rounded animate-pulse"><span className='opacity-30'>Loading...</span></div>
      </div>
      <div className="h-40 md:w-[400px] bg-gray-700/50 rounded animate-pulse mb-2"><span className='opacity-30'>Loading...</span></div>
      <div className="h-4 w-24 bg-gray-700 rounded animate-pulse"><span className='opacity-30'>Loading...</span></div>
            <div className="h-4 w-24 bg-gray-700 rounded animate-pulse"><span className='opacity-30'>Loading...</span></div>
            <div className="flex gap-2">
              <div className="h-8 w-24 bg-gray-700 rounded animate-pulse"><span className='opacity-30'>Loading...</span></div>
      <div className="h-8 w-24 bg-gray-700 rounded animate-pulse"><span className='opacity-30'>Loading...</span></div>
    ...
    <>
        ...
        <div className="text-left  2xl:text-[1.1rem] md:text-[1rem] sm:text-[0.9rem] text-[0.8rem]  z-20 text-gray-200  absolute  left-0 top-auto   mt-auto mb-auto gap-2 flex flex-col  h-full justify-end bottom-3 w-full px-10  mx-auto pb-3  inset-0 ">
          ...
          <h3 className='text-2xl md:text-3xl 2xl:text-4xl font-bold uppercase text-white leading-tight'>
            {serieOne?.name}</h3>
          <div className='flex flex-wrap items-center gap-3 2xl:text-[1.1rem] md:text-[1rem] sm:text-[0.9rem] text-[0.8rem]'>
            <span className='font-semibold text-black bg-yellow-500 2xl:text-[1.1rem] md:text-[1rem] sm:text-[0.9rem] text-[0.8rem]  p-0 m-0 pt-0 pb-0 rounded-sm'>{serieOne?.vote_average?.toFixed(1) ? <span className='border-1 border-gray-300 bg-yellow-500  p-[0.2rem] m-0 pt-0 pb-0 rounded-sm'>{serieOne?.vote_average?.toFixed(1) }</span> :  (
             <span className='bg-yellow-300/70 rounded w-[36px] h-[1rem] inline-block animate-pulse'></span>
            )}</span>
            <span className='font-semibold text-[var(--textSecondary)]/95 2xl:text-[1.1rem] md:text-[1rem] sm:text-[0.9rem] text-[0.8rem]  p-0 m-0 pt-0 pb-0 rounded-sm'>{serieOne?.first_air_date ? <span className='border-1 border-gray-300  p-[0.2rem] m-0 pt-0 pb-0 rounded-sm'>{new Date(serieOne?.first_air_date).getFullYear()}</span> :  (
              <span className='bg-gray-500/20 rounded w-[30px] h-[1rem] inline-block animate-pulse'></span>
            )}</span>
            <span className='font-semibold text-[var(--textSecondary)]/95 2xl:text-[1.1rem] md:text-[1rem] sm:text-[0.9rem] text-[0.8rem] p-0 m-0 pt-0 pb-0 rounded-sm'>{serieOne?.origin_country?.[0] ? <span className='border-1 border-gray-300   p-[0.2rem] m-0 pt-0 pb-0 rounded-sm'>{serieOne?.origin_country?.[0]}</span>:  (
              <span className='bg-gray-500/20 rounded w-[30px] h-[1rem] inline-block animate-pulse'></span>
            )}</span>
            <span className='font-semibold text-[var(--textSecondary)]/95 2xl:text-[1.1rem] md:text-[1rem] sm:text-[0.9rem] text-[0.8rem] p-0 m-0 pt-0 pb-0 rounded-sm'>{currentSerieDetails?.number_of_seasons ? <span className=' border-1 border-gray-300   p-[0.2rem] m-0 pt-0 pb-0 rounded-sm'>{currentSerieDetails?.number_of_seasons + ' Seasons'}</span> :  (
               <span className='bg-gray-500/20 rounded w-[30px] h-[1rem] inline-block animate-pulse'></span>
              )}
              </span>
            </div>
          <p className=' 2xl:text-[1.1rem] md:text-[1rem] sm:text-[0.9rem] text-[0.8rem] text-[var(--textSecondary)]  line-clamp-7 max-w-[360px]'>{serieOne?.overview ? serieOne?.overview  :
          ...
          <div className='flex flex-col items-start mt-3 font-semibold text-[var(--textSecondary)]'>
            <span className='2xl:text-[1.1rem] md:text-[1rem] sm:text-[0.9rem] text-[0.8rem]'>
