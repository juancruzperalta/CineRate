  ...
  return (
    <div className='flex flex-col mb-4 min-h-screen w-full px-10 overflow-hidden text-[var(--textPrimary)]'>
          ...
          <div className={`grid grid-rows-1 gap-4 items-center justify-center w-full mt-2`} style={{gridTemplateColumns: `repeat(${getCant()}, minmax(0, 1fr))`}}>
            ...
            <button className={`cursor-pointer mt-2 text-md rounded-md px-5 py-2 bg-[var(--colorAccent)]/20 text-[var(--textPrimary)] hover:bg-[var(--colorAccent)]/40 transition ${isVisible ? 'hidden' : 'inline-flex'} ${visible?.length > 0 ? 'inline-flex' : 'hidden'}`} onClick={() => setIsVisible(true)}>Ver Más</button>
          </div>
        </div>
      </div>
  )
