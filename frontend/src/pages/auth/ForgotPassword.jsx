...
export const ForgotPassword = () => {
 ...
 <div className='px-10 mt-12 w-full h-screen flex items-center justify-center'>
      <form action="" className='flex items-center justify-center flex-col gap-3 bg-[#272b2f]/80 rounded-sm shadow-sm shadow-gray-900 px-8 py-2 h-[240px] w-[400px] relative'>
        ...
        <input type="email" className={`w-full px-2 py-2 bg-[#090c0f] text-gray-100 rounded-sm disabled:opacity-50`} placeholder='Email' id="email" required />
        <input type="button" value="Forgot password" className='bg-white w-full rounded-sm p-2 text-black uppercase font-semibold text-[0.9rem] disabled:opacity-80'  placeholder='Forgot Password' onClick={() => forgotPassword(document.querySelector("#email").value)}/>
      </form>
      </div>
  )
}
