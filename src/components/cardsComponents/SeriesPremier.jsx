...
export const SeriesPremier = ({cant, premiere=false}) => {
  ...
  return (
    <>
      <div className='flex flex-col items-center justify-center overflow-hidden relative w-full h-full gap-4 mt-4'>
        <div className={`gap-2 h-full grid  w-full ${premiere ? 'grid-cols-8' : `grid-rows-1 grid-cols-${cant}`} items-center justify-between`}>
      ...
      {premiereSeries?.slice(0,cant).map(serie => (
        ...
        <div className='h-20'>
          <Link to="/premiere" className={`border-1 border-gray-500 rounded-md py-1 px-6 cursor-pointer  hover:border-gray-700 hover:bg-[var(--bgSecondary) ${premiere ? 'hidden' : `show`}`}>
