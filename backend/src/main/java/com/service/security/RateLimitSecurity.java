...
public class RateLimitSecurity {
  ...
  Map<String, long[]> emailAndTimeAndCount = new HashMap<>();
    ...
    Map<String, long[]> rateLimitIP = new HashMap<>();
  // long[0] = timeStart
  // long[1] = count
  public void createIP(String ip){
    rateLimitIP.put(ip, new long[]{System.currentTimeMillis(),0});
}
  ...
  public boolean checkEmail(String email) {
   ...
   public boolean checkIp(String ip) {
    int countMax = 15;
    long[] timeStart = rateLimitIP.get(ip);
    if (timeStart == null) {
      return false;
    }
    timeStart[1]++;
  ...
  public boolean existsIP(String ip) {
      return rateLimitIP.containsKey(ip);
  }
}
